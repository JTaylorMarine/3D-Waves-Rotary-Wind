...
H0=100;                    % Depth (m)
dx=5000;                  % Spatial x grid step (m);
dy=5000;                  % Spatial y grid step (m)
...
BotSmooth=5;               % Smoothing parameter of the bottom (1,2,3,...)
                   % INCOMING WAVE
AmpZ=5;                    % Amplitude of the incident wave (m)
...
AmpSeiche=0;               % Initial wave amplitude in the epicentre (m)
                   % NUMERICAL INPUT PARAMETERS
Lat=0;                     % Latitude (deg)
N=500;                     % Number of temporal steps
...
AmpV=AmpZ*g*k*f/(sig^2-f^2);    % Amplitude of U velocity (m/sec)
                       ...
                       %% TOPOGRAPHY 1: SEAMOUNT
% Hg=90;                     % Height of the bank  (m)
% XWidth=1500000;            % X-Width of the bank (m)
% YWidth=1500000;            % Y-Width of the bank (m)
% Xcentre=0;                 % X-positioin of the bank centre
% Ycentre=1000000;           % X-positioin of the bank centre                       
for j=1:J 
      for i=1:I
          H(j,i)=H0;
      end
...
% XQuake=500000;                 % X position of the epicentre (m)
% YQuake=500000;                 % Y position of the epicentre (m)
% AmpQuake=5;                    % Initial wave amplitude in the epicentre (m)
% WidthQuake=100000;             % Half-Width of the earthquake (m)
if AmpQuake>0
       for i=1:I
       for j=1:J 
